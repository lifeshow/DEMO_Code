...
+ (void)keyWith:(void(^)(NSString *pubKey, NSString *priKey))block {
    ...
    result = RSA_generate_key_ex(rsa, 512, bne, NULL);
    //n:模,e:公钥指数，d:私钥指数
...
+(NSString *)privateKetExp
{
    return @"03";
}


#pragma mark -------根据私钥推出指数-----------
+(NSString *)expPrivateKey:(NSString *)privateKey
{
   RSA *rsa = [self openssl_privateKeyFromBase64:privateKey];
   
    char * s = BN_bn2hex(rsa->e);
    
    return [NSString stringWithFormat:@"%s",s];
    
}

#pragma mark -----------根据私钥推出公私钥--------------
+ (void)generPrivaKey:(NSString *)privateKey keyWith:(void(^)(NSString *pubKey, NSString *priKey))block
{
    RSA *rsa = [self openssl_privateKeyFromBase64:privateKey];
    ...
    /* 提取公钥字符串 */
    // 最终存储的地方，所以需要创建一个路径去存储字符串
    NSString *pubPath = [documentsPath stringByAppendingPathComponent:@"PubFiler.txt"];
    FILE* pubWrite = NULL;
    pubWrite = fopen([pubPath UTF8String], "wb");
    if(pubWrite == NULL)
    {
    ...
    /*提取私钥字符串*/
    NSString *priPath = [documentsPath stringByAppendingPathComponent:@"PriFiler.txt"];
    FILE *priWtire = NULL;
    priWtire = fopen([priPath UTF8String], "wb");
...
#pragma mark ----通过公私钥模推出公私钥-----
+(void)openssl_publicKeyFormMod:(NSString *)mod publicKeyExp:(NSString *)pubExp privateKeyExp:(NSString *)priExp keyWith:(void(^)(NSString *pubKey, NSString *priKey))block
{
    RSA * rsa = [self openssl_publicKeyFormMod:mod publicKeyExp:pubExp privateKeyExp:priExp];
...
+ (RSA *)openssl_publicKeyFormMod:(NSString *)mod publicKeyExp:(NSString *)pubExp privateKeyExp:(NSString *)priExp{
    
    RSA * rsa_pub = RSA_new();
    
    const char *N=[mod UTF8String] ;
    const char *E=[pubExp UTF8String];
     const char *D=[priExp UTF8String];
